    ...
    <div className={`bg-surface text-on-surface font-body selection:bg-secondary-container selection:text-on-secondary-container transition-opacity duration-500 ${isVisible && !isExiting ? 'opacity-100' : 'opacity-0'}`}>
      <main className="relative h-screen w-full flex flex-col overflow-hidden max-w-md mx-auto sm:max-w-none sm:mx-0 shadow-lg sm:shadow-none bg-surface-container-lowest">
        ...
        <div className="absolute top-0 left-0 w-full z-10 px-4 py-4 sm:px-6 sm:py-6 flex justify-between items-center bg-transparent">
            ...
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gradient-to-br from-primary to-primary-container flex items-center justify-center text-white font-headline font-extrabold text-base sm:text-lg">S</div>
            <span className="font-headline font-extrabold text-lg sm:text-xl tracking-tighter text-primary">{APP_NAME}</span>
          </div>
          {currentStep < 3 && (
            <button 
        ...
        <footer className="px-4 py-4 pb-6 sm:px-6 sm:py-6 sm:pb-12 bg-white/80 backdrop-blur-md rounded-t-[32px] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)] border-t border-slate-100 z-10 w-full max-w-md mx-auto sm:max-w-none">
          ...
          {currentStep < 3 ? (
            <div className="flex flex-col items-center">
              <button 
                onClick={handleNext}
                className="w-full sm:w-[80%] max-w-[320px] py-3 sm:py-3.5 bg-gradient-to-r from-primary to-primary-container text-white font-headline font-bold rounded-2xl shadow-lg shadow-primary/20 active:scale-95 transition-transform flex items-center justify-center gap-2"
              >
            ...
            <div className="flex flex-col gap-3 items-center">
              <button 
                onClick={() => handleNavigate('/signup')}
                className="w-full sm:w-[80%] max-w-[320px] py-3 sm:py-3.5 bg-gradient-to-r from-primary to-primary-container text-white font-headline font-bold rounded-2xl shadow-lg shadow-primary/20 active:scale-95 transition-transform"
              >
                CREATE ACCOUNT
              </button>
              <button 
                onClick={() => handleNavigate('/login')}
                className="w-full sm:w-[80%] max-w-[320px] py-3 sm:py-3.5 bg-surface-container-high text-on-surface font-headline font-bold rounded-2xl active:scale-95 transition-transform hover:bg-surface-container-highest"
              >
                SIGN IN
              </button>
            </div>
